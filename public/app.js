...
function isDmxSceneCreationPrompt(text = '') {
  ...
  const createIntent = /(crea|crear|cree|nueva|nuevo|create|make|build|design)/.test(value);
  const sceneIntent = /(escena|scene|efecto|effect)/.test(value);
  const lightingIntent = /(dmx|luz|luces|light|lights|iluminaci[oó]n)/.test(value);
  return createIntent && sceneIntent && lightingIntent;
}
...
// Keep the recording headline aligned with the live tool registry count.
